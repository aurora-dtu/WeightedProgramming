...
@[simp]
theorem ωSup_eq_iSup {c : Chain α} :
    ωSup c = ⨆ i, c i := rfl
@[simp]
theorem pi_ωSup_eq_iSup {ι : Type*} {c : Chain (ι → α)} :
    ωSup c = ⨆ i, c i := by
  ext x
  show ωSup (c.map ⟨(· x), _⟩) = _
  simp
@[simp]
theorem orderHom_ωSup_eq_iSup {ι : Type*} [Preorder ι] {c : Chain (ι →o α)} :
    ωSup c = ⨆ i, c i := by ext; simp
@[simp]
theorem orderHom_ωSup_eq_iSup' {ι : Type*} [CompleteLattice ι] {c : Chain (ι →o α)} :
    @ωSup _ OrderHom.omegaCompletePartialOrder c = ⨆ i, c i := by ext; simp
...
theorem opt_ωScottContinuous {α β : Type*} [Order.Frame α] [Order.Frame β]
  ...
  · simp only [𝒟_opt]
    ...
    · intro c
      have := hf.map_ωSup_of_orderHom; simp at this; rw [this]; clear this
      have := hg.map_ωSup_of_orderHom; simp at this; rw [this]; clear this
